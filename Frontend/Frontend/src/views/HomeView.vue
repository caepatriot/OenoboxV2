...
import { buildOliSuggestions, cellarCapacity, extractPlacements, extractUnits, formatCurrency, inventoryValue, occupancyRate, safeArray } from '@/utils/cellarMetrics.js'
    ...
    <div class="oeno-hero mb-6">
      <div class="oeno-hero__content">
        ...
        <div class="d-flex ga-3 flex-wrap mt-4">
          <RouterLink to="/cellar" class="oeno-link-chip oeno-link-chip--light">
            <v-icon size="18">mdi-view-grid-outline</v-icon>
            Open workspace
          </RouterLink>
          <RouterLink to="/tasting" class="oeno-link-chip oeno-link-chip--light">
      ...
      <v-card class="oeno-card" variant="flat">
        <v-card-text class="pa-6">
          ...
          <div class="oeno-action-grid">
            <RouterLink to="/cellar" class="oeno-action-card">
              <v-icon size="26">mdi-home-floor-1</v-icon>
              <span>Workspace</span>
            </RouterLink>
            <RouterLink to="/inventory" class="oeno-action-card">
              <v-icon size="26">mdi-bottle-wine-outline</v-icon>
              <span>Inventory</span>
            </RouterLink>
            <RouterLink to="/cellars" class="oeno-action-card">
              <v-icon size="26">mdi-vector-square</v-icon>
              <span>All cellars</span>
            </RouterLink>
            <RouterLink to="/tasting" class="oeno-action-card">
    ...
    <v-card class="oeno-card mt-6" variant="flat">
      <v-card-text class="pa-6">
        <div class="d-flex align-center justify-space-between mb-4 flex-wrap ga-3">
          ...
          <RouterLink to="/cellar" class="oeno-link-chip">
        ...
        <div class="oeno-card-grid" v-if="cellarPreview.length">
          <div v-for="cave in cellarPreview" :key="cave.id" class="oeno-list-item oeno-list-item--soft">
            ...
            <v-chip class="oeno-chip" variant="flat">{{ Number(cave.temperature ?? 12).toFixed(1) }}°C</v-chip>
          </div>
        </div>
