...
import { useTastingStore } from '@/modules/tasting/store/tasting.store.js'
...
const inferWineTypeFromOption = (opt) => {
  ...
  if (typeof opt === 'string') {
    ...
    if (s.includes('rosÃ©') || s.includes('rose')) return 'rose'
    return s
  }
  ...
  if (typeof opt.value === 'string') {
    ...
    if (label.includes('rosÃ©') || label.includes('rose')) return 'rose'
    return label
  }
...
const submitForm = async () => {
  try {
    ...
    if (isEditMode.value) {
      ...
      alert('DÃ©gustation mise Ã  jour avec succÃ¨s !')
    } else {
      savedTasting = await store.createTasting(apiData)
      myWines.push(savedTasting)
      alert('DÃ©gustation enregistrÃ©e avec succÃ¨s !')
    }

    resetForm()
  } catch (error) {
    console.error("Erreur lors de l'enregistrement:", error)
    alert("Erreur lors de l'enregistrement de la dÃ©gustation. Veuillez rÃ©essayer.")
  }
}
...
const getTastingTitle = (tasting) => {
  ...
  return `${cepages} - ${tasting?.region || 'RÃ©gion inconnue'}`
}
...
const editTasting = async (tasting) => {
  try {
    ...
    alert('Erreur lors du chargement de la dÃ©gustation pour modification.')
  }
}

const deleteTasting = async (tastingId) => {
  if (!confirm('ÃŠtes-vous sÃ»r de vouloir supprimer cette dÃ©gustation ?')) return
  ...
  try {
    ...
    alert('Erreur lors de la suppression de la dÃ©gustation.')
  }
}
...
const populateFormWithTastingData = (tasting) => {
  ...
  else if (wineTypeCode === 'rose') typeLabel = 'RosÃ©'
...
const getSliderLabel = (field, modelValue) => {
  if (field?.name === 'persistance_aromatique') {
    const labels = ['nulle', 'courte', 'moyenne', 'bonne', 'longue', 'trÃ¨s longue', 'infinie']
        ...
        <v-card v-if="showForm" class="ma-2 wine-card" elevation="4">
          ...
          <v-card-text>
            <v-form>
              <v-text-field
                  variant="outlined"
                  label="Titre de la dÃ©gustation"
                  v-model="newTitle"
                  :rules="titleRules"
                  required
        ...
        <v-list class="wine-list">
          ...
          >
            ...
            <v-list-item-subtitle class="wine-subtitle">
              {{ tasting.wineType }} â€¢ {{ tasting.region }} â€¢ {{ tasting.noteFinale }}
            ...
            <v-card-actions class="pa-4 justify-center">
              ...
              >
                ...
                <span class="d-none d-sm-inline">
                  {{ isEditMode ? 'Mettre Ã  jour' : 'Enregistrer la dÃ©gustation' }}
                </span>
