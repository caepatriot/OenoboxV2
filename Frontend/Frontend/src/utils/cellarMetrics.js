...
export function buildOliSuggestions({ caves = [], placements = [] }) {
  ...
  if (warmCellars.length) {
    ...
    suggestions.push({
      ...
      description: `${cellar.name} is at ${safeNumber(cellar.temperature, 0)}°C. Oli would keep an eye on that zone for long-term aging bottles.`,
      tone: 'warning',
    })
  }
  ...
  if (humidCellars.length) {
    ...
    suggestions.push({
      ...
      description: `${cellar.name} is around ${safeNumber(cellar.humidity, 0)}% humidity. Great for corks, but worth monitoring for labels and storage materials.`,
      tone: 'info',
    })
  }
  ...
  if (!suggestions.length) {
    suggestions.push({
      ...
      description: `${totalBottles} bottles are tracked across ${safeArray(caves).length} cellar spaces. Oli is ready to guide the next improvements.`,
      tone: 'success',
    })
  }
