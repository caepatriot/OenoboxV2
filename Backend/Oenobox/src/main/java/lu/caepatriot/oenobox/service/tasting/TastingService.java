...
import lu.caepatriot.oenobox.entity.wine.WineType;
import lu.caepatriot.oenobox.entity.tasting.Tasting;
import lu.caepatriot.oenobox.repository.tasting.TastingRepository;
import lu.caepatriot.oenobox.dto.wine.WineTypeDto;
import lu.caepatriot.oenobox.entity.wine.Cepage;
import lu.caepatriot.oenobox.dto.tasting.TastingDto;
...
public class TastingService {
    ...
    private TastingDto convertToDto(Tasting tasting) {
        ...
        if (tasting.getAromesNature() != null) {
            Map<String, List<String>> aromesMap = new HashMap<>();
            for (TastingAromesNature an : tasting.getAromesNature()) {
                aromesMap.put(an.getId().getCategory(), an.getNotes());
            }
            dto.setAromesNature(aromesMap);
        }
    ...
    private Tasting convertToEntity(TastingDto dto) {
        ...
        if (dto.getAromesNature() != null) {
            List<TastingAromesNature> aromesList = dto.getAromesNature().entrySet().stream()
                .map(entry -> new TastingAromesNature(
                    new TastingAromesNatureId(entry.getKey(), tasting.getId()),
                    entry.getValue()
                ))
                .collect(Collectors.toList());
            tasting.setAromesNature(aromesList);
        }
